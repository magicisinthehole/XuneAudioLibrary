...
void test_format(const char* filename) {
    ...
    CHECK(title && strcmp(title, "Test Track") == 0, "Title = 'Test Track' (got: '%s')", title ?: "(null)");

    auto* album = xune_meta_album(handle);
    CHECK(album && strcmp(album, "Test Album") == 0, "Album = 'Test Album' (got: '%s')", album ?: "(null)");

    auto* genre = xune_meta_genre(handle);
    CHECK(genre && strcmp(genre, "Electronic") == 0, "Genre = 'Electronic' (got: '%s')", genre ?: "(null)");
    ...
    if (ac >= 2) {
        ...
        CHECK(a0 && strcmp(a0, "Polo & Pan") == 0, "Artist[0] = 'Polo & Pan' (got: '%s')", a0 ?: "(null)");
        CHECK(a1 && strcmp(a1, "Metronomy") == 0, "Artist[1] = 'Metronomy' (got: '%s')", a1 ?: "(null)");
    }

    auto* display = xune_meta_artist_display(handle);
    CHECK(display && strstr(display, "Polo") && strstr(display, "Metronomy"),
          "Artist display contains both (got: '%s')", display ?: "(null)");
    ...
    if (aac >= 1) {
        ...
        CHECK(aa0 && strcmp(aa0, "Polo & Pan") == 0, "AlbumArtist[0] = 'Polo & Pan' (got: '%s')", aa0 ?: "(null)");
    }
    ...
    if (mbac >= 2) {
        ...
        CHECK(mb0 && strstr(mb0, "1d9ec7ea"), "MB ArtistID[0] starts with 1d9ec7ea (got: '%s')", mb0 ?: "(null)");
        CHECK(mb1 && strstr(mb1, "93eb7110"), "MB ArtistID[1] starts with 93eb7110 (got: '%s')", mb1 ?: "(null)");
    }
