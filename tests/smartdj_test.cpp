...
TEST_F(ModelInferenceTest, InferenceMatchesReference) {
    ...
    bool ok = onnx.RunInference(chunk.data(), 1, n_mels, chunk_frames, output);
    ASSERT_TRUE(ok);
    ASSERT_EQ(output.size(), 768u);
...
TEST_F(ModelInferenceTest, BatchedInferenceConsistent) {
    ...
    ASSERT_TRUE(onnx.RunInference(chunk.data(), 1, n_mels, chunk_frames, single_output));
    ...
    ASSERT_TRUE(onnx.RunInference(batch_input.data(), 3, n_mels, chunk_frames, batch_output));
    ASSERT_EQ(batch_output.size(), 3u * 768u);
...
TEST_F(EmbeddingPipelineTest, FreeNullResultIsSafe) {
    xune_embedding_free_result(nullptr);  // Should not crash
}
...
TEST_F(EmbeddingPipelineTest, InferProducesCorrectDimensions) {
    ...
    int err = xune_embedding_infer(session, xune_embedding_mel_data(mel),
                                    xune_embedding_mel_chunk_count(mel), &result);
    ASSERT_EQ(err, XUNE_EMBEDDING_OK);
    ASSERT_NE(result, nullptr);

    EXPECT_EQ(xune_embedding_result_chunk_count(result), 1);
    EXPECT_EQ(xune_embedding_result_dimensions(result), 768);
    EXPECT_NE(xune_embedding_result_data(result), nullptr);
...
TEST_F(EmbeddingPipelineTest, EndToEndMatchesReference) {
    ...
    // Phase 2: infer
    xune_embedding_result_t* result = nullptr;
    ASSERT_EQ(xune_embedding_infer(session, xune_embedding_mel_data(mel),
                                    xune_embedding_mel_chunk_count(mel), &result),
              XUNE_EMBEDDING_OK);
    ...
    float sim = CosineSimilarity(data, ref_embeddings_.data(), 768);
    EXPECT_GT(sim, 0.99f)
...
TEST_F(EmbeddingPipelineTest, LongerAudioProducesMultipleChunks) {
    ...
    EXPECT_EQ(xune_embedding_result_chunk_count(result), mel_chunks);
    EXPECT_EQ(xune_embedding_result_dimensions(result), 768);
...
TEST_F(EmbeddingPipelineTest, CrossTrackBatchInference) {
    ...
    xune_embedding_result_t* result_a = nullptr;
    ASSERT_EQ(xune_embedding_infer(session, xune_embedding_mel_data(mel_a),
                                    chunks_a, &result_a),
              XUNE_EMBEDDING_OK);

    xune_embedding_result_t* result_b = nullptr;
    ASSERT_EQ(xune_embedding_infer(session, xune_embedding_mel_data(mel_b),
                                    chunks_b, &result_b),
              XUNE_EMBEDDING_OK);
    ...
    xune_embedding_result_t* result_combined = nullptr;
    ASSERT_EQ(xune_embedding_infer(session, combined_mel.data(),
                                    total_chunks, &result_combined),
              XUNE_EMBEDDING_OK);

    ASSERT_EQ(xune_embedding_result_chunk_count(result_combined), total_chunks);

    const float* combined_data = xune_embedding_result_data(result_combined);
    const float* separate_a = xune_embedding_result_data(result_a);
    const float* separate_b = xune_embedding_result_data(result_b);
    ...
    float sim_a = CosineSimilarity(combined_data, separate_a, 768);
    ...
    xune_embedding_free_result(result_combined);
    xune_embedding_free_result(result_b);
    xune_embedding_free_result(result_a);
